...
LOG_FILE = os.path.join(LOGFILE_DIR, f"ai_wrapper_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.log")
...
def run_cycle(run_type, script_path, stamp_file):
    ...
    output_log = os.path.join(LOGFILE_DIR, f"output_{run_type}_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.txt")
    ...
    gfs_stamp = os.path.join(gfs_dir, "gfs_downloaded_today.stamp")
