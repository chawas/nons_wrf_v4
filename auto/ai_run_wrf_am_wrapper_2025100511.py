...
def run_cycle(run_type, script_path, stamp_file):
    ...
    send_email(f"{run_type} run started: ai_run_wrf_am_wrapper", f"{run_type} automation run started at {start_time} UTC")
    ...
    send_email(f"{run_type} run completed: ai_run_wrf_am_wrapper", summary, [ZIPPED_OUTPUT])
