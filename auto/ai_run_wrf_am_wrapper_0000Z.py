...
script_path = os.path.join(SCRIPT_DIR, SCRIPT_NAME_0000Z)
...
if __name__ == "__main__":
    ...
    while not is_connected():
        ...
        send_email("WRF-AM run started: ai_run_wrf_am_wrapper_0000Z", f"No internet connection. Retrying in 10 mins...")
        time.sleep(600)  # 5 minutes

    start_time = datetime.now()
    logging.info("AI wrapper WRF run started.")
    send_email("WRF-AM run started: ai_run_wrf_am_wrapper_0000Z", f"WRF automation run started at {start_time.strftime('%Y-%m-%d %H:%M:%S')}.")
    ...
    send_email("WRF-AM run completed: 0000Z", summary, attachments)
