...
def plot_accum(lats, lons, rain, start_t, end_t, outdir, hours, cities, latS, latN, lonW, lonE):
    """Plot accumulated rainfall for given interval with reversed colour scale:
       light brown → white → blue → green → purple → dark brown for heavy rain.
    """

    # Rainfall class boundaries (mm)
    clevs = [0, 1, 5, 10, 25, 50, 75, 100, 150, 200, 300, 400, 600]

    # Custom reversed rainfall colormap
    # Low rain: light brown → white → light blue → green → purple → dark brown (heavy rain)
    # Custom rainfall colormap in the order: white → light blue → green → purple → blue → black → brown
    colors = [
        (1.0, 1.0, 1.0),   # white – near zero rainfall
        (0.6, 0.9, 0.6),   # light green

        (0.2, 0.7, 0.3),   # green
        (0.6, 0.8, 1.0),   # light blue
        (0.0, 0.4, 0.8),   # blue
        (0.4, 0.2, 0.8),   # purple(Hex
        (0.35, 0.0, 0.4),  # dark purple
        (0.0, 0.0, 0.0),   # black

    ]


    # Interpolate the custom colors smoothly across rainfall intervals
    cmap = mcolors.LinearSegmentedColormap.from_list("rainfall_reversed", colors, N=len(clevs))
    ...
    # --- Interval folder naming ---
    if hours == 1:
        folder, label = "1", "1-hour"
    elif hours == 12:
        folder, label = "12", "12-hour"
    elif hours == 24:
        folder, label = "24", "24-hour"
    elif hours == 240:
        folder, label = "10_day", "10-day"
    else:
        folder, label = str(hours), f"{hours}-hour"
