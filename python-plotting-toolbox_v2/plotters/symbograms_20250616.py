...
def create_symbograms_mat_south1(df, outdata_path):
    ...
    for city in df.columns.levels[0]:
        if city in symbogram_cities:
            ...
            ground_min = pd.DataFrame(mins - 3.5)
            ground_min.columns = ['g_min']

            # Ensure the index is clean and consistent
            ground_min.index = ground_min.index.astype('datetime64[ns]').date

            print(f"{city}: Lengths — temps={len(temps)}, symbs={len(symbs)}, ground_min={len(ground_min)}")
            ...
            # Add g_min only if lengths match
            if len(ground_min) == len(tot):
                tot = pd.merge(tot, ground_min, left_index=True, right_index=True)
            else:
                print(f"Skipping g_min merge for {city} due to length mismatch")
            #tot = pd.merge(tot, ground_min, left_index=True, right_index=True)
            tot.rename(columns = {'t_x':'max'}, inplace=True)
            tot.rename(columns = {'t_y':'min'}, inplace=True)
            print("Columns in tot:", tot.columns)
...
def create_symbograms_main_city_centers_01(df, outdata_path):
    ...
    # Read the configuration file and create a dictionary for each listed city
    symbogram_cities  = config_tools.readCitiesConfig(locations_file)
    choice_list = ['Harare', 'Bulawayo','Gweru', 'Mutare','Kwekwe', 'Kadoma', 'Masvingo', 'Chinhoyi','Lupane', 'Bindura','Marondera', 'Gwanda']  #list(symbogram_cities.keys())[8:15]
    #choice_list = ['Gwanda']  #list(symbogram_cities.keys())[8:15]
    symbo_divs = ""
    for city in df.columns.levels[0]:
        if city in symbogram_cities:
...
def create_symbograms_main_city_centers_02(df, outdata_path):
    ...
    # Read the configuration file and create a dictionary for each listed city
    symbogram_cities  = config_tools.readCitiesConfig(locations_file)
    choice_list = ['Masvingo', 'Chinhoyi','Lupane', 'Bindura','Marondera', 'Gwanda']  #list(symbogram_cities.keys())[8:15]
    #choice_list = ['Gwanda']  #list(symbogram_cities.keys())[8:15]
    symbo_divs = ""
    for city in df.columns.levels[0]:
        if city in symbogram_cities:
...
def create_symbograms_holiday_resorts_01(df, outdata_path):
    ...
    # Read the configuration file and create a dictionary for each listed city
    symbogram_cities  = config_tools.readCitiesConfig(locations_file)
    choice_list = ['VicFalls', 'Kariba','GreatZimbabwe', 'Nyanga','Vumba', 'Matobo', 'Chimanimani', 'ChinhoyiCaves','Binga', 'HotSprings','HwangeNatPark', 'Gonarezhou']  #list(symbogram_cities.keys())[8:15]
    #choice_list = ['Gwanda']  #list(symbogram_cities.keys())[8:15]
    symbo_divs = ""
    for city in df.columns.levels[0]:
        if city in symbogram_cities:
...
def create_symbograms_holiday_resorts_02(df, outdata_path):
    ...
    # Read the configuration file and create a dictionary for each listed city
    symbogram_cities  = config_tools.readCitiesConfig(locations_file)
    choice_list = ['Chimanimani', 'ChinhoyiCaves','Binga', 'HotSprings','HwangeNatPark', 'Gonarezhou']  #list(symbogram_cities.keys())[8:15]
    #choice_list = ['Gwanda']  #list(symbogram_cities.keys())[8:15]
    symbo_divs = ""
    for city in df.columns.levels[0]:
        if city in symbogram_cities:
...
def create_symbotable(tot, city):
    ...
    <tr class="g_min-temps">
        <td>{int(round(g_min[0], 0))}&deg</td>
        <td>{int(round(g_min[1], 0))}&deg</td>
        <td>{int(round(g_min[2], 0))}&deg</td>
        <td>{int(round(g_min[3], 0))}&deg</td>
        <td>{int(round(g_min[4], 0))}&deg</td>
    </tr>
