...
def make_related_constants(constants):
    if 'cp_d' in constants and 'cp_v' in constants and 'R_d' not in constants:
        constants['R_d'] = constants['cp_d'] - constants['cv_d']
        constants['R_v'] = constants['cp_v'] - constants['cv_v']
    if 'cp_i' in constants and 'cv_i' not in constants:
        constants['cv_i'] = constants['cp_i']
    if 'cp_l' in constants and 'cv_l' not in constants:
        constants['cv_l'] = constants['cp_l']
    if all([v in constants for v in ('L_s', 'L_v')]) and 'L_f' not in constants:
...
class Var:
    ...
    def repr(v, formatting='%g', skip=[]):
        ...
        return ", ".join([r"$%s=%g%s$" % (Var.names[i], v[i], units.get(Var.names[i], '')) for i in range(Var.NUM) if Var.names[i] not in skip])
...
class ThermodynamicParcelIntegration():
    ...
    def __init__(self, z, p, T, rel_humid=None, qv=None, constants=default_constants):
        ...
        if sum([qv is not None, rel_humid is not None]) != 1:
